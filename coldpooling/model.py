...
def get_model(model_name):
    ...
    if model is None:
        raise NotImplementedError(
            "This model name is not included yet. Please choose from 'last_baseline', 'linear', 'dense', 'conv', 'lstm', or 'AR lstm'."
        )
    return model
...
def compile_and_fit(
    model, window, patience=20, MAX_EPOCHS=150
):  # lstm_units, out_steps, num_features,
...
def define_models(OUT_STEPS, num_features, CONV_WIDTH=3, lstm_units=100):
    ...
    AR_lstm_model = FeedBack(
        units=lstm_units, num_features=num_features, out_steps=OUT_STEPS
    )
...
def plot_variable_importance(lstm_model, window):
    ...
    variable_importance_and_names_sorted = sorted(
        zip(variable_importance, window.input_columns)
    )
    variable_importance_sorted, names_sorted = zip(
        *variable_importance_and_names_sorted
    )

    plt.figure(figsize=(4, 4), dpi=100)
    plt.barh(
        np.arange(len(variable_importance)), variable_importance_sorted, color="teal"
    )
